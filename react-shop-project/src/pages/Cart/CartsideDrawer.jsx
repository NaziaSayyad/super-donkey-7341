...
function Cartdrawer({cartitem}){
    ...
    const getdata = (cartitem) =>{
        return axios.get(`https://intense-journey-23599.herokuapp.com/api/shoe?id=${cartitem}`)
    }
const fetchdata =() =>{
    getdata(cartitem).then((res) => setdata([...data,res.data]));
}
    useEffect(() =>{
        fetchdata()
    } , [cartitem])
               ...
               <DrawerHeader borderBottomWidth='1px' display='flex' gap='3'>
                    ...
                    <Container>
                        ...
                        <LinkBox  h='50px' colorScheme='white' color='white' fontWeight='bold' bg='#000063'
                            border='1px'  variant='solid' w='full' textAlign='center'> <Link to='/'> Checkout/View Cart  
                             </Link>
                     </LinkBox>
                    </Container>
                    {/* <Container> */}
